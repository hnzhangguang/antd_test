...
import './TextareaItem1.css';

class TextareaItem2 extends Component {
	constructor(props) {
		super(props);
	}
	componentWillMount() {}
			...
			<div>
				...
				<div>
					...
					<List renderHeader={() => 'Whether is controlled'}>
						<TextareaItem {...getFieldProps('control')} title="受控组件" placeholder="controlled" />
						<TextareaItem title="非受控组件" placeholder="please input content" clear />
					</List>
					<List renderHeader={() => 'Auto / Fixed height'}>
						<TextareaItem {...getFieldProps('note3')} title="高度自适应" autoHeight labelNumber={5} />
						<TextareaItem {...getFieldProps('note1')} rows={3} placeholder="fixed number of lines" />
					</List>
...
const TextareaItem1 = createForm()(TextareaItem2);

export default TextareaItem1;
